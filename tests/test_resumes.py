...
async def test_upload_resume(client_with_token):
    with patch("app.resumes.service.PdfService") as pdf_mock, \
        ...
        response = await client_with_token.post("/resumes", files=files)
        assert response.status_code == 200
        
async def test_get_my_resume_not_found(client_with_token):
    response = await client_with_token.get("/resumes/me")
    assert response.status_code == 404
    
async def test_get_my_resume(client_with_token):
    with patch("app.resumes.service.PdfService") as pdf_mock, \
         patch("app.resumes.service.StorageService") as storage_mock, \
         patch("app.core.embedding_service.EmbeddingService") as embedding_mock:
